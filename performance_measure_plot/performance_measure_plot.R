...
OHA_Performance_Measure_plot <- function(df, pop = "Total", time_unit = "Month", goal, metric_name = "", interactive = TRUE) {
  ...
  
{
  ...
  if(!is.character(pop)) {
    ...
    cli_abort(c(
      "{.var pop} must be a {.cls character} vector of length one.",
      "x" = "The value supplied to {.var pop} was of class {.cls {class(pop)}}."
    ))
  ...
  if(!is.character(time_unit)) {
    ...
    cli_abort(c(
      "{.var time_unit} must be a {.cls character} vector of length one.",
      "x" = "The value supplied to {.var time_unit} was of class {.cls {class(time_unit)}}."
    ))
  ...
  if(!is.numeric(goal)) {
    ...
    cli_abort(c(
      "{.var goal} must be a {.cls numeric} vector of length one.",
      "x" = "The value supplied to {.var goal} was of class {.cls {class(goal)}}."
    ))
    ...
    if(!is.character(metric_name)) {
      ...
      cli_abort(c(
        "{.var metric_name} must be a {.cls character} vector of length one.",
        "x" = "The value supplied to {.var metric_name} was of class {.cls {class(metric_name)}}."
      ))
  ...
  series <- rlang::sym(ifelse(pop == "Pediatric", "ped_percent", 
                              ifelse(pop == "Adult", "adult_percent", "tot_percent")
                              )
                       )
  
  denom_series <- rlang::sym(ifelse(pop == "Pediatric", "ped_denom", 
                                    ifelse(pop == "Adult", "adult_denom", "tot_denom")
                                    )
                             )
  
  group_vars <- if (time_unit == "Month") {
    
    rlang::syms(c("year", "quarter", "month")
                )
    
  } else if (time_unit == "Quarter") {
    
    rlang::syms(c("year", "quarter")
                )
  ...
  time_var <- rlang::sym(ifelse(time_unit == "Month", "month",
                                ifelse(time_unit == "Quarter", "quarter", "year")))
  
  x_axis_labels <- if (time_unit == "Month") {month.abb
  } else if (time_unit == "Quarter") {quarter_label_list
  } else if (time_unit == "Year") {as.character(year_list)}
  
  x_axis_breaks <- if (time_unit == "Month") {1:12
  } else if (time_unit == "Quarter") {1:4
  } else if (time_unit == "Year") {year_list}
  
  color_palette <- if (time_unit == "Year") {"#005595"} else {c("#c0c0c0", "#005595")}
  group_var <- if (time_unit == "Year") {NULL} else {as.factor(analytic_df$year)}
  
  a <- ggplot(analytic_df, aes(x = !!time_var, y = !!series, group = group_var,
                               text = sprintf("%s: %s, %i<br>Denominator: %s<br>Percent: %s", 
                                              time_unit, 
                                              x_axis_labels[!!time_var], 
                                              analytic_df$year,
                                              prettyNum(round(!!denom_series, digits = 1),
                                                        big.mark = ",",
                                                        preserve.width = "individual"), # scales::comma() has been superseded
                                              scales::percent(!!series, 
                                                              accuracy = 0.1) # use scales::percent() instead for readability
                               )
                               )
              ) + 
    geom_point(size = 2, aes(color = as.factor(analytic_df$year))) +
    geom_line(linewidth = 1, aes(color = as.factor(analytic_df$year))) +
    geom_hline(yintercept = goal, linetype = "dashed", color = "#EC891D") +
    scale_x_continuous(labels = x_axis_labels, breaks = x_axis_breaks) + 
    scale_y_continuous(labels = scales::label_percent(accuracy = 0.1), 
                       limits=c(0,1)) + # utilize scales::label_percent directly, percent_format was superseded
