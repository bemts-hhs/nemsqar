...
section of the National Emergency Medical Services Information System
(NEMSIS). It contains example patient demographics and scene-related
details that can be used for testing various functions within the
`nemsqar` package. Users are encouraged to test these functions with
this dataset, but results should not be interpreted as meaningful. Some
outputs may be nonsensical, which is expected since this data is only
intended to demonstrate the expected structure of input data.
...
- `Patient Age Units (ePatient.16)`:
  ...
  (chr) The units of measurement for patient age (e.g., Years, Months,
  Hours, etc.).
