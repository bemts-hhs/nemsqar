...
asthma_01 <- function(df,
  ...
  initial_population <- df |>
    ...
    dplyr::filter(patient_age_in_years_col >= 2)
  ...
  total_population <- initial_population |>
    dplyr::summarize(
      ...
      denominator = n(),
      prop = sum(beta_agonist_check, na.rm = T) / n(),
      prop_label = pretty_percent(sum(beta_agonist_check, na.rm = T) / n(),
                                  n_decimal = 0.01),
      ...
    )
  ...
  # summary
  asthma.01 <- dplyr::bind_rows(adult_population, peds_population, total_population)
