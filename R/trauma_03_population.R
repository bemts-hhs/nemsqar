...
#'
#' Identifies key categories to records that are 911 request for patients whose pain score was 
#' lowered during the EMS encounter. based on specific criteria and calculates related ECG measures. 
#' This function segments the data by age into adult and pediatric populations.
#' 
#' @section Data Assumptions:
#' -  If a user passes the `evitals_27_initial_col` and `evitals_27_last_col`, the function
#' will utilize those values to do calculations for the numerator.
#' - In the case that those values are `NULL`, the `evitals_27_col` must be passed
#' and the function will do its best to find the first and last pain scores based on
#' the `evitals_01_col` values.
...
#' @param incident_date_col <['tidy-select'][dplyr_tidy_select]> The column indicating the incident date. Must be of class `Date` or similar. Default is `NULL`.
#' @param patient_DOB_col <['tidy-select'][dplyr_tidy_select]> The column representing the patient's date of birth. Must be of class `Date` or similar. Default is `NULL`.
...
#' @param evitals_27_col <['tidy-select'][dplyr_tidy_select]> The column for the pain scale score. Default is `NULL`.
#' @param evitals_27_initial_col <['tidy-select'][dplyr_tidy_select]> The column for the initial pain scale score. Default is `NULL`.
#' @param evitals_27_last_col <['tidy-select'][dplyr_tidy_select]> The column for the last pain scale score. Default is `NULL`.
...
trauma_03_population <- function(df = NULL,
  ...
  filter_counts <- tibble::tibble(
    ...
    count = c(
      ...
      sum(computing_population$PAIN_SCALE_TIME, na.rm = T),
