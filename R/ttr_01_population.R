...
#' by EMS during which a basic set of vital signs is documented based on specific criteria 
#' and calculates related ECG measures. This function segments the data by age into adult and pediatric populations.
#' 
#' @section Data Assumptions:
#' 
#' This function assumes the input dataset contains the *initial* or complete set of vital signs for 
#' - respiratory rate, 
  ...
  if (
    ...
    vitals_data <- vitals_table |> 
      ...
      dplyr::filter( 
       ...
       ( !is.na({{ evitals_23_col }}) | grepl(pattern = avpu_responses, x = {{ evitals_26_col }}, ignore.case = T) )
    ...
    filter_counts <- tibble::tibble(
      ...
      count = c(
        ...
        sum(computing_population$NO_TRANSPORT, na.rm = T),
        sum(computing_population$NOT_CARDIAC_ARREST, na.rm = T),
        sum(computing_population$VITALS, na.rm = T),
        nrow(adult_pop),
        nrow(peds_pop),
